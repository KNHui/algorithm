/*
	...
	오차 제곱의 합은 부분 합과 부분 제곱 합을 알면 시간 복잡도 O(1) 만에 알 수 있다.
	"부분합[idx]"와 "부분 제곱 합[idx]"는 'nums' 수열의 구간 "[0, idx]"의 부분합과 부분 제곱 합을 나타낸다.
	이때, 구간 "[A, B]"의 부분 합은 "부분합[B] - 부분합[A - 1]"이고,
	부분 제곱 합은 "부분 제곱 합[B] - 부분 제곱 합[A - 1]"이다.

	오차 "num - 평균"의 제곱은 "num^2 - 2 * 평균 * num + 평균^2"이 된다.
	위 식를 이용해 구간 "[A, B]"의 오차 제곱의 합을 구할 수 있다.
	'num^2'은 구간 "[A, B]"의 부분 제곱 합이 되고,
	'num'은 구간 "[A, B]"의 부분 합이 되기 때문이다.
